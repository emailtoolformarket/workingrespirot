  ...
  return (
    <div className="bg-dots-light min-h-screen bg-slate-100">
      <Sidebar
        ...
        emailsSent={stats?.emails_sent_this_month ?? 45200}
      />
          ...
          {error ? (
            <div className="animate-fade-up mx-auto mt-16 max-w-md rounded-xl border border-rose-200 bg-white p-8 text-center shadow-sm">
              ...
              <p className="mt-1.5 text-sm text-slate-500">
                Check your connection to the API, then try again.
              </p>
              <button
                type="button"
                onClick={() => void load()}
            ...
            <>
              ...
              <div className="animate-fade-up flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-slate-500">
                    Here's how your audience engaged this week.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() =>
                    push("The campaign editor ships in the next milestone — hang tight.", "info")
                  }
              ...
              <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
                <StatCard
                  ...
                  value={stats.total_subscribers.toLocaleString("en-US")}
                  icon={<Users className="h-5 w-5" />}
                  delta={{ label: "+3.8% vs last mo.", positive: true }}
                  spark={[11200, 11800, 12400, 12100, 13200, 13900, stats.total_subscribers]}
                  accent="indigo"
                />
                <StatCard
                  index={1}
                  label="Emails sent this month"
                  value={stats.emails_sent_this_month.toLocaleString("en-US")}
                  icon={<Send className="h-5 w-5" />}
                  delta={{ label: "+12.4% vs last mo.", positive: true }}
                  spark={[28400, 31200, 33800, 30100, 38400, 41900, stats.emails_sent_this_month]}
                  accent="sky"
                />
                <StatCard
                  index={2}
                  ...
                  delta={{ label: "-1.2 pts", positive: false }}
                  spark={[26.1, 25.4, 25.8, 24.9, 25.2, 24.1, stats.average_open_rate]}
                  accent="amber"
                />
                <StatCard
                  index={3}
                  label="Average click rate"
                  value={`${stats.average_click_rate}%`}
                  icon={<MousePointerClick className="h-5 w-5" />}
                  delta={{ label: "+0.4 pts", positive: true }}
                  spark={[2.7, 2.9, 2.8, 3.0, 3.1, 3.0, stats.average_click_rate]}
                  accent="emerald"
                />
              </div>
              ...
              <p className="animate-fade-in mt-8 pb-2 text-center text-xs text-slate-400" style={{ animationDelay: "600ms" }}>
                Relay MVP · GET /api/dashboard/stats · data refreshes with every sign-in
              </p>
            </>
          ) : null}
        </main>
...
function DashboardSkeleton() {
  return (
    <div aria-busy="true" aria-label="Loading dashboard">
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="h-44 animate-pulse rounded-xl border border-slate-200 bg-white" style={{ animationDelay: `${i * 120}ms` }} />
        ))}
      </div>
      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-3">
